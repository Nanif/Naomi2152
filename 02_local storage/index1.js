...
function initCurrentUser(){
    //get all the users to a list
    let allUsers = localStorage.getItem("allUsers");
    allUsers = (allUsers) ? JSON.parse(allUsers) : [];
    console.log(allUsers);
    let currentUser=allUsers[allUsers.length-1];
    let nameOfCurrentUser=currentUser.fullName;
    document.getElementById("userName").innerHTML+=nameOfCurrentUser;
}
